...
def build_tariffs_keyboard() -> InlineKeyboardMarkup:
    rows = []
    for key in ["7_days", "30_days", "180_days", "360_days", "forever"]:
        rows.append([
            InlineKeyboardButton(
                text=f"{period_title(key)} — {RUB_PRICES[key]}₽ / {RUB_PRICES[key]}⭐",
                callback_data=f"plan|{key}"
            )
        ])
    if MAIN_BOT_USERNAME:
        rows.append([InlineKeyboardButton(text=" Назад в основной бот", url=main_bot_url())])
